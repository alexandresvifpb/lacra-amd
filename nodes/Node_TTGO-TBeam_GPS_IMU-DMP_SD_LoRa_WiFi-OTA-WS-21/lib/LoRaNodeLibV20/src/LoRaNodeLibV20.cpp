...
uint16_t numberSendMessage = 0;

LinkedList<String> listRecvMessage = LinkedList<String>();
uint16_t numberRecvMessage = 0;
...
// Inputs: nenhum
// Return: boolean
boolean LoRaNodeLib::requestSyncTime(uint64_t epochTime) {

    // return true;
    // Serial.println((unsigned long) epochTime);

    // if (now() > LORA_EPOCH_TIME_2019) return true;
    if (epochTime > LORA_EPOCH_TIME_2019) return true;
...
// Verifica se listSendMessage esta vazia
// Input: void
// Return: boolean TRUE se a lista não estiver vazia
int LoRaNodeLib::avaliableSendMessage(void)
{
    return numberSendMessage;
}

// Retorna o registro 0 de listSendMessage removendo-o da lista
// Inputs: void
// Return: String com o registro
String LoRaNodeLib::getNextSendMessage(void) {
    String _sendMessage;
...
// Salva em um estrutura list a String
// Inputs: String a ser salvo na estrutura list lisSendMessage
// Return: void
void LoRaNodeLib::addNewMessageSend(String message) {

    Serial.print("LORA_ADDS: ");
    Serial.println(message);

    listSendMessage.add(message);
    numberSendMessage++;

    // Serial.println(listSendMessage.size());
}
