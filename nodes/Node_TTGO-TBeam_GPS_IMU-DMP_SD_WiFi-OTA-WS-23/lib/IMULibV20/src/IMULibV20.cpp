...
boolean IMULib::begin(void)
{
    ...
    imuSendLoopDelayS = IMU_SEND_LOOP_DELAY_S * 60 * ( 1000 / imuTaskDelayMS );
    // imuSendLoopDelayS = IMU_SEND_LOOP_DELAY_S * 60 * 1000;
...
// Inputs: void
// Return: EulerAngles_t
// static void IMULib::toEulerianAngle(float w, float x, float y, float z, double& roll, double& pitch, double& yaw)
//                                 .q0                   .q1                   .q2                   .q3
// toEulerianAngle(imu.calcQuat(imu.qw), imu.calcQuat(imu.qx), imu.calcQuat(imu.qy), imu.calcQuat(imu.qz), roll, pitch, yaw);
EulerAngles_t IMULib::toEulerianAngle(Quaternion_t quaternion)
{
