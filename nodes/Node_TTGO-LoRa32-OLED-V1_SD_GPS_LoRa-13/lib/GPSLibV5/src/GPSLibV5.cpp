...
/*    
    if (gpsDebugMain) Serial.println("---> GPS read()");

    while (Serial1.available()) {
    
        if (gpsDebugMain) Serial.println("---> GPS.read().available()");

        int temp = Serial1.read();

        if (gpsDebugMain) Serial.println(char(temp));

        // gpsNEO.encode(Serial1.read());
        gpsNEO.encode(temp);
        if (gpsNEO.location.isUpdated()) {

            if (gpsDebugMain) Serial.println(gpsNEO.location.lat());
