...
// Serial                                 D:\downloads\doutorado\2019\prototipos\firmwares\lacra-amd\gateways\Gateway_TTGO_LoRa32-OLED_V1_Temp-Umid_MQTT_LoRa-16
#define MESSAGE_INITIAL                 ("C:\\downloads\\doutorado\\2019\\prototipos\\firmwares\\lacra-amd\\getways\\Gateway_TTGO_LoRa32-OLED_V1_Temp-Umid_MQTT_LoRa-16")          // localizacao do projeto
...
void Task1code( void * pvParameters ) {
    ...
    for(;;) {
        ...
        thpa.run();
        if (thpa.avaliable()) {
            // String strTHPADatas = thpa.getStrDataTHPA(millis());
            // String strTHPADatas = thpa.getStrDataTHPA(millis(), addressIdESP);
