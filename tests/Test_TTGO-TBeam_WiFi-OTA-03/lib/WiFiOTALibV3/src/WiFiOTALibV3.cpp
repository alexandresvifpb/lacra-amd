...
uint64_t lastTimeReconnection, currentTimeReconnection;
uint32_t tsTimeReconnection = 60000;
...
boolean WiFiOTALib::begin(void)
{
    ...
    wifi_assid = "Node-" + getMacAddress();

    //define wifi como AP + station (AP + estação)
    ...
    // Inicializa o objeto wifi passando o ssid e pass
    WiFi.softAP(wifi_assid.c_str(), wifi_apass.c_str());

    WiFi.onEvent(wifiEventConnected);
...
{
    if ( (millis() - lastTimeReconnection) > tsTimeReconnection )
    {
        WiFi.reconnect();
        lastTimeReconnection = millis();
    }
...
// Retorna o numero do atual status da conexao
// Inputs: void
// Return: int - status da conexao
int WiFiOTALib::getStatusConnection(void)
{
    return WiFi.status();
}

// Retorna o numero de dispositivo conectados
// Inputs: void
// Return: int - numero de conexoes
int WiFiOTALib::getNumberStationConnectedAP(void)
{
    // Serial.print("Number of Station Connected: ");
